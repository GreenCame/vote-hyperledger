...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if function == "recordElected" {
		return s.recordElected(APIstub, args)
	} else if function == "queryAllElected" {
		return s.queryAllElected(APIstub, args)
	}
...
func (s *SmartContract) queryVote(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if voteAsBytes == nil {
		return shim.Error("Could not locate your vote")
	}
...
func (s *SmartContract) recordVote(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var vote = Vote{ Elected: args[1], Location: args[2], Timestamp: args[3], Holder: args[4] }

	voteAsBytes, _ := json.Marshal(vote)
	err := APIstub.PutState(args[0], voteAsBytes)
	if err != nil {
		return shim.Error(fmt.Sprintf("Failed to record vote catch: %s", args[0]))
	}
